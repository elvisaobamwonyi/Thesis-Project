...
sfh1 = pd.read_csv(sfh,skiprows=48,nrows=10, index_col=0)
th1 = pd.read_csv(th,skiprows=48,nrows=10, index_col=0)
mfh1 = pd.read_csv(mfh,skiprows=48,nrows=10, index_col=0)
ab1 = pd.read_csv(ab,skiprows=48,nrows=10, index_col=0)
...
A_ref = [173.2, 100.3, 468.6, 3322.0]   # reference area [m2]
...
Fnu = [0.84, 0.87, 0.92, 0.93]           # temp reduction (high thermal conductivity
...
Aw_est = [5.0, 11.7, 40.6, 229.0]     # window area in east direction
Aw_st = [16.6, 0.0, 0.0, 34.0]      # window area in south direction
Aw_wst = [5.0, 11.7, 46.0, 229.0]     # window area in east direction
Aw_nrt = [7.6, 0.0, 0.0, 53.0]     # window area in east direction
...
heating_demand78 = pd.DataFrame({'period': np.arange(len(weather)),
                              'sfh': demand_sfh,
                               'th': demand_th,
                               'mfh': demand_mfh,
                               'ab': demand_ab
...
heating_demand78.to_csv(("output/heat demand_1969-1978.csv"), index=False)


heatflow78 = pd.DataFrame({"Heat transmission": ["Heat loss", "Heat gain", "Heat demand"],
...
heatflow78.to_csv(("output/heatflow_1969-1978.csv"), index=False)

# Heat Demand Per Square Meter
total_demand78 = round(sum([sum(Q_demand_sfh), sum(Q_demand_th),   # total space heating demand for all building types kWh/a
                    sum(Q_demand_mfh), sum(Q_demand_ab)]))
total_demand78_sqm = round(total_demand78 / sum(A_ref),0)
heatflow78_sqm = pd.DataFrame({'Heatiing demand [kWh/m^2.a]':[heatflow78["SFH[KWh/a]"][2],heatflow78["TH[KWh/a]"][2],
                                       heatflow78["MFH[KWh/a]"][2],heatflow78["AB[KWh/a]"][2], total_demand78_sqm],
                              'Building types': ['SFH', 'TH', 'MFH', 'AB', 'Total Demand']}, index=None)

a_x = sns.barplot(x='Building types',y = 'Heatiing demand [kWh/m^2.a]',
                  width=0.25,data=heatflow78_sqm, color='seagreen', label='from 1969-1978')
for i in a_x.containers:
    a_x.bar_label(i,)
plt.xlabel('Building types', fontsize=18)
plt.ylabel('Heating demand [KWh/m^2.a]', fontsize=18)
plt.yticks([0, 25, 50, 75, 100, 125, 150, 175],fontsize=18)
plt.xticks([r for r in range(len(heatflow78_sqm))],
        ['SFH', 'TH', 'MFH', 'AB', 'Total Demand'])
plt.title('Space heating demand per square meter', fontsize=18)
plt.legend()
plt.ticklabel_format()
plt.tight_layout()
plt.savefig(f'output/Demand_sqm from 1969-1978')
plt.show()


#fig = plt.rcParams["figure.figsize"] = (10,6)
plt.stackplot(heating_demand78.index, heating_demand78["sfh"], heating_demand78["th"], heating_demand78["mfh"],
              heating_demand78["ab"], colors=['gold', 'seagreen', 'red', 'limegreen'], labels=['SFH', 'TH', 'MFH', 'AB'])
plt.ylim(0, None)
...
plt.tight_layout()
plt.ticklabel_format()
plt.savefig(f'output/1969-1978')
plt.show()
...
plt.legend()
plt.grid()
plt.savefig(f'output/year load profile 1969-1978')
plt.show()
...
plt.legend()
plt.grid()
plt.savefig(f'output/LDC_HL_1969-1978')
plt.show()
