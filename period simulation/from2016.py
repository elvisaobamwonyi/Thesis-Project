...
sfh1 = pd.read_csv(sfh,skiprows=120,nrows=10, index_col=0)
th1 = pd.read_csv(th,skiprows=120,nrows=10, index_col=0)
mfh1 = pd.read_csv(mfh,skiprows=120,nrows=10, index_col=0)
ab1 = pd.read_csv(ab,skiprows=120,nrows=10, index_col=0)
...
# Tabula Data
teta = 20.0                              # indoor temp
A_ref = [186.8, 195.8, 1305.0, 0]        # reference area [m2] for SFH,TH, MFH and AB
btr_ex = 1.0                             # adjustment soil factor for construction bordering on external air
btr_exw = 0.5                            # adjustment soil factor for construction bordering on wall
btr_floor = 0.5                          # adjustment soil factor of unheated cellar/construction bordering on soil
Fnu = [0.95, 1.0, 0.98, 0]               # temp reduction SFH,TH, MFH and AB (high thermal conductivity
base_temp = 12.0                         # Base temp [°C]
DU_tbr = 0.05                            # thermal bridge U-value
...
A_roof = [sfh1.iloc[0][0], th1.iloc[0][0], mfh1.iloc[0][0], ab1.iloc[0][0]]        # roof envelope area 1 [m2]
A_roof1 = [sfh1.iloc[1][0], th1.iloc[1][0], mfh1.iloc[1][0], ab1.iloc[1][0]]       # roof envelope area 2 [m2]
U_roof = [sfh1.iloc[0][1], th1.iloc[0][15], mfh1.iloc[0][15], ab1.iloc[0][15]]     # actual u value for roof1 [W/(m2K)]
U_roof1 = [sfh1.iloc[1][1], th1.iloc[1][15], mfh1.iloc[1][15], ab1.iloc[1][15]]    # actual u value for roof2 [W/(m2K)]
...
#print(U_roof[0], U_roof1[0], U_wall1[0], U_wall2[0], U_floor[0], U_floor1[0], U_win[0],U_win1[0], U_door[0])
#print(A_roof[0], A_roof1[0], A_wall1[0], A_wall2[0], A_floor[0], A_floor1[0], A_win[0],A_win1[0], A_door[0])
#print(U_roof[1], U_roof1[1], U_wall1[1], U_wall2[1], U_floor[1], U_floor1[1], U_win[1],U_win1[1], U_door[1])
#print(A_roof[1], A_roof1[1], A_wall1[1], A_wall2[1], A_floor[1], A_floor1[1], A_win[1],A_win1[1], A_door[1])
#print(U_roof[2], U_roof1[2], U_wall1[2], U_wall2[2], U_floor[2], U_floor1[2], U_win[2],U_win1[2], U_door[2])
#print(A_roof[2], A_roof1[2], A_wall1[2], A_wall2[2], A_floor[2], A_floor1[2], A_win[2],A_win1[2], A_door[2])
...
for i in range(0, len(weather)):
    ...
    # transmission loss SFH
        qtr_loss_coeff_sfh = (
                            (round(U_wall1[0] * A_wall1[0] * btr_ex) + round(U_wall2[0] * A_wall2[0] * btr_ex)
    ...
    # transmission loss TH
        qtr_loss_coeff_th = (
                            (round(U_wall1[1] * A_wall1[1] * btr_ex) + round(U_wall2[1] * A_wall2[1] * btr_exw)
    ...
    # transmission loss AB
        ...
        Q_hloss_ab.append(round(combine_loss3))
    else:
...
ggl = 0.50       # solar energy transmittance
...
Aw_hor = [0, 2.6, 0, 0]         # window area in east direction
Aw_est = [2.7, 0.0, 60.5, 0.0]     # window area in east direction
Aw_st = [22.6, 8.8, 19.4, 0.0]      # window area in south direction
Aw_wst = [13.0, 0.0, 163.7, 0.0]     # window area in east direction
Aw_nrt = [3.7, 16.7, 0.0, 0.0]     # window area in east direction
...
time_cons_mfh = round((cm * A_ref[2]) / (coeff_tr_res_mfh + coeff_ve_res_mfh),2)     # Building time constant
...
print(cm,time_cons_ab,aH_ab, Yh_ab, n_ff_ab)'''
n_ff_ab = 1
...
heating_demand16 = pd.DataFrame({'period': np.arange(len(weather)),
                              'sfh': demand_sfh,
                               'th': demand_th,
                               'mfh': demand_mfh,
                               'ab': demand_ab
                               }).set_index('period')
#print(heating_demand16)
"""
#plt.show()

heating_demand16.to_csv(("output/heat demand_from 2016.csv"), index=False)


heatflow16 = pd.DataFrame({"Heat transmission": ["Heat loss", "Heat gain", "Heat demand"],
                              ...
                              "AB[KWh/a]": [round(sum(Q_hloss_ab)), round(sum(Q_hgain_ab)), round(sum(Q_demand_ab))],
                             }, index=None)
heatflow16.to_csv(("output/heatflow_from 2016.csv"), index=False)

total_demand16 = round(sum([sum(Q_demand_sfh), sum(Q_demand_th),   # total space heating demand for all building types kWh/a
                    sum(Q_demand_mfh), sum(Q_demand_ab)]))
total_demand16_sqm = round(total_demand16 / sum(A_ref),0)

#print(total_demand)
print(total_demand16_sqm)

# Heat Demand Per Square Meter
heatflow16_sqm = pd.DataFrame({'Heatiing demand [kWh/m^2.a]':[heatflow16["SFH[KWh/a]"][2],heatflow16["TH[KWh/a]"][2],
                                       heatflow16["MFH[KWh/a]"][2],sum(Q_demand_ab), total_demand16_sqm],
                              'Building types': ['SFH', 'TH', 'MFH', 'AB', 'Total Demand']}, index=None)

a_x = sns.barplot(x='Building types',y = 'Heatiing demand [kWh/m^2.a]',
                  width=0.25,data=heatflow16_sqm, color='g')
for i in a_x.containers:
    a_x.bar_label(i,)
plt.xlabel('Building types', fontsize=14)
plt.ylabel('Heating demand [KWh/m^2.a]', fontsize=14)
plt.xticks([r for r in range(len(heatflow16_sqm))],
        ['SFH', 'TH', 'MFH', 'AB', 'Total Demand'])
plt.title('Space heating demand per square meter reference area', fontsize=14)
plt.legend()
plt.ticklabel_format()
plt.tight_layout()
plt.savefig(f'output/Demand_sqm from 2016')
plt.show()

fig = plt.rcParams["figure.figsize"] = (10,6)
plt.stackplot(heating_demand16.index, heating_demand16["sfh"], heating_demand16["th"], heating_demand16["mfh"],
              heating_demand16["ab"], colors=['gold', 'limegreen', 'red', 'blue'], labels=['SFH', 'TH', 'MFH', 'AB'])
plt.ylim(0, None)
plt.xlim(0, len(demand_sfh))
plt.xlabel("Time Period (h)", fontdict={'fontsize': 16})
plt.title(f'Space Heating Demand Profile', fontdict={'fontsize': 16})
plt.ylabel('Power (KW)', fontdict={'fontsize': 16})
...
plt.grid()
plt.savefig(f'output/from 2016')
plt.tight_layout()
plt.show()

# TODO Yearly Duration Curve
heating_demand16['Heatlost'] = heating_demand16[list(heating_demand16.columns)].sum(axis=1) # Add the two columns together
heating_demand_array = heating_demand16['Heatlost'].values  # sorting values from large to small
...
plt.legend()
plt.grid()
plt.savefig(f'output/year load profile from 2016')
plt.show()
...
plt.legend()
plt.grid()
plt.savefig(f'output/LDC_HL_from 2016')
plt.show()
"""