...
sfh1 = pd.read_csv(sfh,skiprows=108,nrows=10, index_col=0)
th1 = pd.read_csv(th,skiprows=108,nrows=10, index_col=0)
mfh1 = pd.read_csv(mfh,skiprows=108,nrows=10, index_col=0)
ab1 = pd.read_csv(ab,skiprows=108,nrows=10, index_col=0)
#print(th1)
#print(mfh1)
...
A_ref = [186.5, 195.8, 1305.0, 0]   # reference area [m2]
...
Fnu = [0.90, 0.96, 0.98, 0]           # temp reduction (high thermal conductivity
...
for i in range(0, len(weather)):
    ...
    # transmission loss SFH
        qtr_loss_coeff_sfh = (
                            (round(U_wall1[0] * A_wall1[0] * btr_ex) + round(U_wall2[0] * A_wall2[0] * btr_exw[0])
    ...
    # transmission loss TH
        qtr_loss_coeff_th = (
                            (round(U_wall1[1] * A_wall1[1] * btr_ex) + round(U_wall2[1] * A_wall2[1] * btr_exw[1])
...
ggl = 0.60       # solar energy transmittance
...
heating_demand15 = pd.DataFrame({'period': np.arange(len(weather)),
                              'sfh': demand_sfh,
                               'th': demand_th,
                               'mfh': demand_mfh,
                               'ab': demand_ab
...
heating_demand15.to_csv(("output/heat demand_2010-2015.csv"), index=False)


heatflow15 = pd.DataFrame({"Heat transmission": ["Heat loss", "Heat gain", "Heat demand"],
...
total_demand = round(sum([sum(Q_demand_sfh), sum(Q_demand_th),   # total space heating demand for all building types kWh/a
                    sum(Q_demand_mfh), sum(Q_demand_ab)]))
total_demand_sqm = round(total_demand / sum(A_ref),0)

#print(total_demand)
print(total_demand_sqm)
heatflow15.to_csv(("output/heatflow_2010-2015.csv"), index=False)

# Heat Demand Per Square Meter

heatflow15_sqm = pd.DataFrame({'Heatiing demand [kWh/m^2.a]':[heatflow15["SFH[KWh/a]"][2],heatflow15["TH[KWh/a]"][2],
                                       heatflow15["MFH[KWh/a]"][2],0, total_demand_sqm],
...
#br1 = np.arange(len(heatflow15_sqm))

# Make the plot
'''
plt.bar(br1, heatflow15_sqm, color ='b', width=barWidth,
        edgecolor ='grey', label ='from 2010-2015')
'''
#width = barWidth,

# Adding Xticks
a_x = sns.barplot(x='Building types',y = 'Heatiing demand [kWh/m^2.a]',
                  width=0.25,data=heatflow15_sqm, color='b')
for i in a_x.containers:
    a_x.bar_label(i,)
plt.xlabel('Building types', fontsize=18)
plt.ylabel('Heating demand [KWh/m^2.a]', fontsize=18)
plt.yticks([0, 10, 20, 30, 40, 50, 60, 70, 80],fontsize=18)
plt.xticks([r for r in range(len(heatflow15_sqm))],
        ['SFH', 'TH', 'MFH', 'AB', 'Total Demand'])
plt.title('Space heating demand per square meter', fontsize=18)
plt.legend()
plt.ticklabel_format()
plt.tight_layout()
plt.savefig(f'output/Demand_sqm 2010-2015')
plt.show()

#fig = plt.rcParams["figure.figsize"] = (10,6)
plt.stackplot(heating_demand15.index, heating_demand15["sfh"], heating_demand15["th"], heating_demand15["mfh"],
              heating_demand15["ab"], colors=['red', 'limegreen', 'gold', 'blue'], labels=['SFH', 'TH', 'MFH', 'AB'])
plt.ylim(0, None)
...
plt.ticklabel_format()
plt.tight_layout()
plt.savefig(f'output/2010-2015')
plt.show()

# TODO Yearly Duration Curve
heating_demand15['Heatlost'] = heating_demand15[list(heating_demand15.columns)].sum(axis=1) # Add the two columns together
heating_demand_array = heating_demand15['Heatlost'].values  # sorting values from large to small
...
plt.legend()
plt.grid()
plt.savefig(f'output/year load profile 2010-2015')
plt.show()
...
plt.legend()
plt.grid()
plt.savefig(f'output/LDC_HL_2010-2015')
plt.show()
