...
sfh1 = pd.read_csv(sfh,skiprows=24,nrows=10, index_col=0)
th1 = pd.read_csv(th,skiprows=24,nrows=10, index_col=0)
mfh1 = pd.read_csv(mfh,skiprows=24,nrows=10, index_col=0)
ab1 = pd.read_csv(ab,skiprows=24,nrows=10, index_col=0)
...
A_ref = [111.1, 149.6, 632.3, 1602.7]   # reference area [m2]
...
Fnu = [0.80, 0.84, 0.90, 0.91]    # temp reduction (high thermal conductivity
...
Aw_est = [3.2, 0.0, 45.6, 128.3]     # window area in east direction
Aw_st = [8.6, 25.6, 3.8, 38.3]      # window area in south direction
Aw_wst = [3.2, 0.0, 45.6, 128.3]     # window area in east direction
Aw_nrt = [3.3, 21.0, 3.8, 0.0]     # window area in east direction
...
heating_demand57 = pd.DataFrame({'period': np.arange(len(weather)),
                              'sfh': demand_sfh,
                               'th': demand_th,
                               'mfh': demand_mfh,
                               'ab': demand_ab
...
heating_demand57.to_csv(("output/heat demand_1949-1957.csv"), index=False)


heatflow57 = pd.DataFrame({"Heat transmission": ["Heat loss", "Heat gain", "Heat demand"],
...
heatflow57.to_csv(("output/heatflow_1949-1957.csv"), index=False)

# Heat Demand Per Square Meter
total_demand57 = round(sum([sum(Q_demand_sfh), sum(Q_demand_th),   # total space heating demand for all building types kWh/a
                    sum(Q_demand_mfh), sum(Q_demand_ab)]))
total_demand57_sqm = round(total_demand57 / sum(A_ref),0)
heatflow57_sqm = pd.DataFrame({'Heatiing demand [kWh/m^2.a]':[heatflow57["SFH[KWh/a]"][2],heatflow57["TH[KWh/a]"][2],
                                       heatflow57["MFH[KWh/a]"][2],heatflow57["AB[KWh/a]"][2], total_demand57_sqm],
                              'Building types': ['SFH', 'TH', 'MFH', 'AB', 'Total Demand']}, index=None)

a_x = sns.barplot(x='Building types',y = 'Heatiing demand [kWh/m^2.a]',
                  width=0.25,data=heatflow57_sqm, color='teal', label='from 1949-1957')
...
plt.xticks([r for r in range(len(heatflow57_sqm))],
        ['SFH', 'TH', 'MFH', 'AB', 'Total Demand'])
plt.title('Space heating demand per square meter', fontsize=18)
plt.legend()
plt.ticklabel_format()
plt.tight_layout()
plt.savefig(f'output/Demand_sqm from 1949-1957')
plt.show()


#fig = plt.rcParams["figure.figsize"] = (10,6)
plt.stackplot(heating_demand57.index, heating_demand57["sfh"], heating_demand57["th"], heating_demand57["mfh"],
              heating_demand57["ab"], colors=['gold', 'deeppink', 'teal', 'lawngreen'], labels=['SFH', 'TH', 'MFH', 'AB'])
plt.ylim(0, None)
...
plt.grid()
plt.savefig(f'output/1949-1957')
plt.ticklabel_format()
plt.tight_layout()
plt.show()
...
plt.legend()
plt.grid()
plt.savefig(f'output/year load profile 1949-1957')
plt.show()
...
plt.legend()
plt.grid()
plt.savefig(f'output/LDC_HL_1949-1957')
plt.show()