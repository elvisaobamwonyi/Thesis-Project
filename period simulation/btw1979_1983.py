...
sfh1 = pd.read_csv(sfh,skiprows=60,nrows=10, index_col=0)
th1 = pd.read_csv(th,skiprows=60,nrows=10, index_col=0)
mfh1 = pd.read_csv(mfh,skiprows=60,nrows=10, index_col=0)
ab1 = pd.read_csv(ab,skiprows=60,nrows=10, index_col=0)
...
A_ref = [215.6, 108.3, 654.0, 3107.5]   # reference area [m2]
...
Fnu = [0.88, 0.87, 0.93, 0.95]           # temp reduction (high thermal conductivity
...
Aw_est = [8.7, 0.0, 46.3, 0.0]     # window area in east direction
Aw_st = [6.0, 11.9, 3.4, 215.0]      # window area in south direction
Aw_wst = [8.7, 0.0, 46.3, 0.0]     # window area in east direction
Aw_nrt = [3.6, 8.3, 3.4, 172.0]     # window area in east direction
...
heating_demand83 = pd.DataFrame({'period': np.arange(len(weather)),
                              'sfh': demand_sfh,
                               'th': demand_th,
                               'mfh': demand_mfh,
                               'ab': demand_ab
...
heating_demand83.to_csv(("output/heat demand_1979-1983.csv"), index=False)


heatflow83 = pd.DataFrame({"Heat transmission": ["Heat loss", "Heat gain", "Heat demand"],
...
heatflow83.to_csv(("output/heatflow_1979-1983.csv"), index=False)

# Heat Demand Per Square Meter
total_demand83 = round(sum([sum(Q_demand_sfh), sum(Q_demand_th),   # total space heating demand for all building types kWh/a
                    sum(Q_demand_mfh), sum(Q_demand_ab)]))
total_demand83_sqm = round(total_demand83 / sum(A_ref),0)
heatflow83_sqm = pd.DataFrame({'Heatiing demand [kWh/m^2.a]':[heatflow83["SFH[KWh/a]"][2],heatflow83["TH[KWh/a]"][2],
                                       heatflow83["MFH[KWh/a]"][2],heatflow83["AB[KWh/a]"][2], total_demand83_sqm],
                              'Building types': ['SFH', 'TH', 'MFH', 'AB', 'Total Demand']}, index=None)

a_x = sns.barplot(x='Building types',y = 'Heatiing demand [kWh/m^2.a]',
                  width=0.25,data=heatflow83_sqm, color='violet', label='from 1979-1983')
for i in a_x.containers:
    a_x.bar_label(i,)
plt.xlabel('Building types', fontsize=18)
plt.ylabel('Heating demand [KWh/m^2.a]', fontsize=18)
plt.yticks([0, 20, 40, 60, 80, 100,120],fontsize=18)
plt.xticks([r for r in range(len(heatflow83_sqm))],
        ['SFH', 'TH', 'MFH', 'AB', 'Total Demand'])
plt.title('Space heating demand per square meter reference area', fontsize=16)
plt.legend()
plt.ticklabel_format()
plt.tight_layout()
plt.savefig(f'output/Demand_sqm from 1979-1983')
plt.show()


#fig = plt.rcParams["figure.figsize"] = (10,6)
plt.stackplot(heating_demand83.index, heating_demand83["sfh"], heating_demand83["th"], heating_demand83["mfh"],
              heating_demand83["ab"], colors=['violet', 'yellow', 'brown', 'aquamarine'], labels=['SFH', 'TH', 'MFH', 'AB'])
plt.ylim(0, None)
...
plt.yticks([0, 50, 100, 150,200],fontsize=18)
...
plt.tight_layout()
plt.ticklabel_format()
plt.savefig(f'output/1979-1983')
plt.show()
...
plt.legend()
plt.grid()
plt.savefig(f'output/year load profile 1979-1983')
plt.show()
...
plt.tight_layout()
plt.legend()
plt.savefig(f'output/LDC_HL_1979-1983')
plt.show()
