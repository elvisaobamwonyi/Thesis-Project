...
sfh1 = pd.read_csv(sfh,skiprows=12,nrows=10, index_col=0)
th1 = pd.read_csv(th,skiprows=12,nrows=10, index_col=0)
mfh1 = pd.read_csv(mfh,skiprows=12,nrows=10, index_col=0)
ab1 = pd.read_csv(ab,skiprows=12,nrows=10, index_col=0)
...
A_ref = [302.5, 112.8, 385.0, 1484.0]   # reference area [m2]
...
Fnu = [0.83, 0.86, 0.88, 0.91]    # temp reduction (high thermal conductivity
...
Aw_est = [9.1, 0.0, 9.5, 126.9]     # window area in east direction
Aw_st = [22.0, 13.6, 33.7, 24.7]      # window area in south direction
Aw_wst = [9.1, 0.0, 9.5, 126.9]     # window area in east direction
Aw_nrt = [12.2, 7.9, 18.5, 0.0]     # window area in east direction
...
heating_demand48 = pd.DataFrame({'period': np.arange(len(weather)),
                              'sfh': demand_sfh,
                               'th': demand_th,
                               'mfh': demand_mfh,
                               'ab': demand_ab
...
heating_demand48.to_csv(("output/heat demand_1919-1948.csv"), index=False)


heatflow48 = pd.DataFrame({"Heat transmission": ["Heat loss", "Heat gain", "Heat demand"],
...
heatflow48.to_csv(("output/heatflow_1919-1948.csv"), index=False)

# Heat Demand Per Square Meter
total_demand48 = round(sum([sum(Q_demand_sfh), sum(Q_demand_th),   # total space heating demand for all building types kWh/a
                    sum(Q_demand_mfh), sum(Q_demand_ab)]))
total_demand48_sqm = round(total_demand48 / sum(A_ref),0)
heatflow48_sqm = pd.DataFrame({'Heatiing demand [kWh/m^2.a]':[heatflow48["SFH[KWh/a]"][2],heatflow48["TH[KWh/a]"][2],
                                       heatflow48["MFH[KWh/a]"][2],heatflow48["AB[KWh/a]"][2], total_demand48_sqm],
                              'Building types': ['SFH', 'TH', 'MFH', 'AB', 'Total Demand']}, index=None)

a_x = sns.barplot(x='Building types',y = 'Heatiing demand [kWh/m^2.a]',
                  width=0.25,data=heatflow48_sqm, color='lawngreen', label='from 1919-1948')
for i in a_x.containers:
    a_x.bar_label(i,)
plt.xlabel('Building types', fontsize=18)
plt.ylabel('Heating demand [KWh/m^2.a]', fontsize=18)
plt.yticks([0, 25, 50, 75, 100, 125, 150, 175, 200], fontsize=18)
plt.xticks([r for r in range(len(heatflow48_sqm))],
        ['SFH', 'TH', 'MFH', 'AB', 'Total Demand'], fontsize=18)
plt.title('Space heating demand per square meter', fontsize=18)
plt.legend()
plt.ticklabel_format()
plt.tight_layout()
plt.savefig(f'output/Demand_sqm from 1919-1948')
plt.show()


#plt.rcParams["figure.figsize"] = (14,6)
plt.stackplot(heating_demand48.index, heating_demand48["sfh"], heating_demand48["th"], heating_demand48["mfh"],
              heating_demand48["ab"], colors=['gold', 'limegreen', 'red', 'blue'], labels=['SFH', 'TH', 'MFH', 'AB'])
plt.ylim(0, None)
...
plt.grid()
plt.savefig(f'output/1919-1948')
plt.show()

# TODO Yearly Duration Curve
heating_demand48['Heatlost'] = heating_demand48[list(heating_demand48.columns)].sum(axis=1) # Add the two columns together
print(heating_demand48)
heating_demand_array = heating_demand48['Heatlost'].values  # sorting values from large to small
...
plt.legend()
plt.grid()
plt.savefig(f'output/year load profile 1919-1948')
plt.show()
...
plt.ylim(0, None)
plt.xlim(0, len(weather))
p.set_title(f"Load duration curve for MFH", fontweight ='bold', fontsize = 15)
p.set_xlabel("Time (hours)", fontweight ='bold', fontsize = 15)
p.set_ylabel("Power (KW)", fontweight ='bold', fontsize = 15)
plt.xticks([0, 744, 1416, 2160, 2880, 3624, 4344, 5088, 5832, 6552, 7296, 8016],
      ['Jan', 'Feb', 'Mar', 'Apr', 'Mai', 'Jun', 'Jul', 'Aug', 'Sep', 'Okt', 'Nov', 'Dec'])
plt.tight_layout()
plt.legend()
plt.grid()
plt.savefig(f'output/LDC_HL_1919-1948')
plt.show()
