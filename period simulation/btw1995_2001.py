...
sfh1 = pd.read_csv(sfh,skiprows=84,nrows=10, index_col=0)
th1 = pd.read_csv(th,skiprows=84,nrows=10, index_col=0)
mfh1 = pd.read_csv(mfh,skiprows=84,nrows=10, index_col=0)
ab1 = pd.read_csv(ab,skiprows=84,nrows=10, index_col=0)
...
A_ref = [121.9, 148.8, 834.9, 0]   # reference area [m2]
...
Fnu = [0.88, 0.92, 0.95, 0]           # temp reduction (high thermal conductivity
...
Aw_est = [3.6, 0.0, 22.7, 0.0]     # window area in east direction
Aw_st = [20.3, 18.2, 77.5, 0.0]      # window area in south direction
Aw_wst = [3.6, 0.0, 22.7, 0.0]     # window area in east direction
Aw_nrt = [5.0, 4.2, 39.9, 0.0]     # window area in east direction
...
heating_demand01 = pd.DataFrame({'period': np.arange(len(weather)),
                              'sfh': demand_sfh,
                               'th': demand_th,
                               'mfh': demand_mfh,
                               'ab': demand_ab
...
heating_demand01.to_csv(("output/heat demand_1995-2001.csv"), index=False)



heatflow01 = pd.DataFrame({"Heat transmission": ["Heat loss", "Heat gain", "Heat demand"],
...
heatflow01.to_csv(("output/heatflow_1995-2001.csv"), index=False)

# Heat Demand Per Square Meter
total_demand01 = round(sum([sum(Q_demand_sfh), sum(Q_demand_th),   # total space heating demand for all building types kWh/a
                    sum(Q_demand_mfh), sum(Q_demand_ab)]))
total_demand01_sqm = round(total_demand01 / sum(A_ref),0)


print(total_demand01_sqm)

# Heat Demand Per Square Meter
heatflow01_sqm = pd.DataFrame({'Heatiing demand [kWh/m^2.a]':[heatflow01["SFH[KWh/a]"][2],heatflow01["TH[KWh/a]"][2],
                                       heatflow01["MFH[KWh/a]"][2],sum(Q_demand_ab), total_demand01_sqm],
                              'Building types': ['SFH', 'TH', 'MFH', 'AB', 'Total Demand']}, index=None)

a_x = sns.barplot(x='Building types',y = 'Heatiing demand [kWh/m^2.a]',
                  width=0.25,data=heatflow01_sqm, color='coral', label='from 1995-2001')
for i in a_x.containers:
    a_x.bar_label(i,)
plt.xlabel('Building types', fontsize=18)
plt.ylabel('Heating demand [KWh/m^2.a]', fontsize=18)
plt.yticks([0, 20, 40, 60, 80,100],fontsize=18)
plt.xticks([r for r in range(len(heatflow01_sqm))],
        ['SFH', 'TH', 'MFH', 'AB', 'Total Demand'])
plt.title('Space heating demand per square meter', fontsize=18)
plt.legend()
plt.ticklabel_format()
plt.tight_layout()
plt.savefig(f'output/Demand_sqm from 1995-2001')
plt.show()


#fig = plt.rcParams["figure.figsize"] = (10,6)
plt.stackplot(heating_demand01.index, heating_demand01["sfh"], heating_demand01["th"], heating_demand01["mfh"],
              heating_demand01["ab"], colors=['firebrick', 'yellow', 'lightgreen', 'blue'], labels=['SFH', 'TH', 'MFH', 'AB'])
plt.ylim(0, None)
...
plt.ticklabel_format()
plt.tight_layout()
plt.savefig(f'output/1995-2001')
plt.tight_layout()
plt.show()
...
plt.legend()
plt.grid()
plt.savefig(f'output/year load profile 1995-2001')
plt.show()
...
plt.legend()
plt.grid()
plt.savefig(f'output/LDC_HL_1995-2001')
plt.show()
