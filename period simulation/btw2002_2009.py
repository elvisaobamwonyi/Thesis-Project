...
sfh1 = pd.read_csv(sfh,skiprows=96,nrows=10, index_col=0)
th1 = pd.read_csv(th,skiprows=96,nrows=10, index_col=0)
mfh1 = pd.read_csv(mfh,skiprows=96,nrows=10, index_col=0)
ab1 = pd.read_csv(ab,skiprows=96,nrows=10, index_col=0)
...
A_ref = [146.5, 151.9, 2190.1, 0]   # reference area [m2]
...
Fnu = [0.90, 0.92, 0.99, 0]           # temp reduction (high thermal conductivity
...
Aw_est = [3.9, 6.6, 79.5, 0.0]     # window area in east direction
Aw_st = [17.3, 19.9, 108.6, 0.0]      # window area in south direction
Aw_wst = [3.9, 6.6, 77.9, 0.0]     # window area in east direction
Aw_nrt = [3.1, 3.2, 42.7, 0.0]     # window area in east direction
...
heating_demand09 = pd.DataFrame({'period': np.arange(len(weather)),
                              'sfh': demand_sfh,
                               'th': demand_th,
                               'mfh': demand_mfh,
                               'ab': demand_ab
...
heating_demand09.to_csv(("output/heat demand_2002-2009.csv"), index=False)


heatflow09 = pd.DataFrame({"Heat transmission": ["Heat loss", "Heat gain", "Heat demand"],
...
heatflow09.to_csv(("output/heatflow_2002-2009.csv"), index=False)

# Heat Demand Per Square Meter
total_demand09 = round(sum([sum(Q_demand_sfh), sum(Q_demand_th),   # total space heating demand for all building types kWh/a
                    sum(Q_demand_mfh), sum(Q_demand_ab)]))
total_demand09_sqm = round(total_demand09 / sum(A_ref),0)

#print(total_demand)
#print(total_demand09_sqm)

# Heat Demand Per Square Meter
heatflow09_sqm = pd.DataFrame({'Heatiing demand [kWh/m^2.a]':[heatflow09["SFH[KWh/a]"][2],heatflow09["TH[KWh/a]"][2],
                                       heatflow09["MFH[KWh/a]"][2],sum(Q_demand_ab), total_demand09_sqm],
                              'Building types': ['SFH', 'TH', 'MFH', 'AB', 'Total Demand']}, index=None)

a_x = sns.barplot(x='Building types',y = 'Heatiing demand [kWh/m^2.a]',
                  width=0.25,data=heatflow09_sqm, color='r', label='from 2002-2009')
...
plt.xticks([r for r in range(len(heatflow09_sqm))],
        ['SFH', 'TH', 'MFH', 'AB', 'Total Demand'])
plt.title('Space heating demand per square meter', fontsize=18)
plt.legend()
plt.ticklabel_format()
plt.tight_layout()
plt.savefig(f'output/Demand_sqm from 2002-2009')
plt.show()

#fig = plt.rcParams["figure.figsize"] = (10,6)
plt.stackplot(heating_demand09.index, heating_demand09["sfh"], heating_demand09["th"], heating_demand09["mfh"],
              heating_demand09["ab"], colors=['orange', 'limegreen', 'purple', 'blue'], labels=['SFH', 'TH', 'MFH', 'AB'])
plt.ylim(0, None)
...
plt.yticks([0, 20, 40, 60, 80],fontsize=12)
...
plt.ticklabel_format()
plt.tight_layout()
plt.savefig(f'output/2002-2009')
plt.tight_layout()
plt.show()

# TODO Yearly Duration Curve
heating_demand09['Heatlost'] = heating_demand09[list(heating_demand09.columns)].sum(axis=1) # Add the two columns together

heating_demand_array = heating_demand09['Heatlost'].values  # sorting values from large to small
...
plt.legend()
plt.grid()
plt.savefig(f'output/year load profile 2002-2009')
plt.show()
...
plt.legend()
plt.grid()
plt.savefig(f'output/LDC_HL_2002-2009')
plt.show()