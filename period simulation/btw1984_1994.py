...
sfh1 = pd.read_csv(sfh,skiprows=72,nrows=10, index_col=0)
th1 = pd.read_csv(th,skiprows=72,nrows=10, index_col=0)
mfh1 = pd.read_csv(mfh,skiprows=72,nrows=10, index_col=0)
ab1 = pd.read_csv(ab,skiprows=72,nrows=10, index_col=0)
...
A_ref = [150.2, 127.6, 778.1, 3107.5]   # reference area [m2]
...
Fnu = [0.86, 0.89, 0.93, 0.90]           # temp reduction (high thermal conductivity
...
Aw_est = [7.4, 9.4, 11.4, 0.0]     # window area in east direction
Aw_st = [12.7, 0.0, 84.2, 215.0]      # window area in south direction
Aw_wst = [7.4, 9.4, 11.4, 0.0]     # window area in east direction
Aw_nrt = [2.1, 0.0, 54.0, 172.0]     # window area in east direction
...
heating_demand94 = pd.DataFrame({'period': np.arange(len(weather)),
                              'sfh': demand_sfh,
                               'th': demand_th,
                               'mfh': demand_mfh,
                               'ab': demand_ab
...
heating_demand94.to_csv(("output/heat demand_1984-1994.csv"), index=False)


heatflow94 = pd.DataFrame({"Heat transmission": ["Heat loss", "Heat gain", "Heat demand"],
...
heatflow94.to_csv(("output/heatflow_1984-1994.csv"), index=False)

# Heat Demand Per Square Meter
total_demand94 = round(sum([sum(Q_demand_sfh), sum(Q_demand_th),   # total space heating demand for all building types kWh/a
                    sum(Q_demand_mfh), sum(Q_demand_ab)]))
total_demand94_sqm = round(total_demand94 / sum(A_ref),0)

#print(total_demand)
#print(total_demand94_sqm)

# Heat Demand Per Square Meter
heatflow94_sqm = pd.DataFrame({'Heatiing demand [kWh/m^2.a]':[heatflow94["SFH[KWh/a]"][2],heatflow94["TH[KWh/a]"][2],
                                       heatflow94["MFH[KWh/a]"][2],heatflow94["AB[KWh/a]"][2], total_demand94_sqm],
                              'Building types': ['SFH', 'TH', 'MFH', 'AB', 'Total Demand']}, index=None)

a_x = sns.barplot(x='Building types',y = 'Heatiing demand [kWh/m^2.a]',
                  width=0.25,data=heatflow94_sqm, color='navy', label='from 1984-1994')
...
plt.xticks([r for r in range(len(heatflow94_sqm))],
        ['SFH', 'TH', 'MFH', 'AB', 'Total Demand'])
plt.title('Space heating demand per square meter', fontsize=18)
plt.legend()
plt.ticklabel_format()
plt.tight_layout()
plt.savefig(f'output/Demand_sqm from 1984-1994')
plt.show()


#fig = plt.rcParams["figure.figsize"] = (10,6)
plt.stackplot(heating_demand94.index, heating_demand94["sfh"], heating_demand94["th"], heating_demand94["mfh"],
              heating_demand94["ab"], colors=['navy', 'yellow', 'red', 'green'], labels=['SFH', 'TH', 'MFH', 'AB'])
plt.ylim(0, None)
...
plt.ticklabel_format()
plt.grid()
plt.savefig(f'output/1984-1994')
plt.show()

# TODO Yearly Duration Curve
heating_demand94['Heatlost'] = heating_demand94[list(heating_demand94.columns)].sum(axis=1) # Add the two columns together
heating_demand_array = heating_demand94['Heatlost'].values  # sorting values from large to small
...
plt.legend()
plt.grid()
plt.savefig(f'output/year load profile 1984-1994')
plt.show()
...
plt.legend()
plt.grid()
plt.savefig(f'output/LDC_HL_1984-1994')
plt.show()
